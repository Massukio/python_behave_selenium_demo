...
def step_impl(context):
    context.browser.get("http://automationpractice.com")

@then("I click login using multiple selectors")
def step_impl(context):
    multiple_selectors = context.browser.find_element_by_xpath('//a[@class="login" and contains(text(), "Sign in")]')
    print("Multi_selectors html:",multiple_selectors.get_attribute("outerHTML"))